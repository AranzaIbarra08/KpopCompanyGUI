...
public class ControladorJYP extends VistaJYP{
	private Empresa JYP; 
	...
	public void muestraMatriz() {
		...
		JYP= new Empresa("JYP", 1996);
		miArchivo=new File("DatosJYP.txt");
		try {
			...
			for(i=1;i<=n; i++) {
				...
				resp=JYP.altaAristas(nombre, ventas, tipo);
			}
			lectura.close();
		...
		for(m=0; m<MAXF; m++)
			for (j=0; j<MAXC; j++)
				matrizAux[m][j].setText(JYP.regresaArtista(m,j));
	}
	
	public ControladorJYP() {
		super();
