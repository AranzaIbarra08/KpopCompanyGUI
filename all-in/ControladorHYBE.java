...
public class ControladorHYBE extends VistaHYBE{
	...
	public void muestraMatriz() {
		...
		miArchivo=new File("DatosHYBE.txt");
		try {
			...
			for(i=1;i<=n; i++) {
				...
				resp=HYBE.altaAristas(nombre, ventas, tipo);
			}
			lectura.close();
		...
		for(m=0; m<MAXF; m++)
			for (j=0; j<MAXC; j++)
				matrizAux[m][j].setText(HYBE.regresaArtista(m,j));
	}
	
	public ControladorHYBE() {
		super();
