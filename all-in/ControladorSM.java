...
public class ControladorSM extends VistaSM {
	...
	public void muestraMatriz() {
		...
		miArchivo=new File("DatosSM.txt");
		try {
			...
			for(i=1;i<=n; i++) {
				...
				resp=SM.altaAristas(nombre, ventas, tipo);
			}
			lectura.close();
		...
		for(m=0; m<MAXF; m++)
			for (j=0; j<MAXC; j++)
				matrizAux[m][j].setText(SM.regresaArtista(m,j));
	}
	
	public ControladorSM() {
		super();
